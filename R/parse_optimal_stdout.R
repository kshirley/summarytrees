...
parse.optimal.stdout <- function(stdout = character()){
  # stdout is the standard output from the C function that Howard wrote (final.c)
  ...
  for (k in 1:K){
    ...
    if (length(w.singles) > 0){
      ...
      single.weights.char <- unlist(strsplit(tmp, "weight ", fixed = TRUE))[seq(2, by = 2, length = length(w.singles))]
    ...
    if (length(w.trees) > 0){
      ...
      trees <- as.numeric(unlist(strsplit(tmp, " ", fixed = TRUE))[seq(1, by = 4, length = length(w.trees))])
    ...
    if (length(w.clusters) > 0){
      ...
      cluster.weights <- sapply(strsplit(stdout[rg][w.clusters], " "), function(x) as.numeric(substr(x[[7]], 1, nchar(x[[7]]) - 1)))
    } else {
      clusters <- NULL
    ...
    x[[k]][, 3] <- rep(1:3, c(length(w.singles), length(w.trees), length(w.clusters)))
