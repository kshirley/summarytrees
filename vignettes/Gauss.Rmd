...
vignette: >
  %\VignetteIndexEntry{Summary Trees Example: Math Genealogy Tree}
...
Next, we compute the maximum entropy $k$-node summary trees for $k = 1, 2, ..., 10$ using three different versions of the algorithm. Setting a maximum size of 10 isn't very realistic -- this is just to illustrate the syntax for all three functions.
...
To compare the resulting 20-node summary trees, for example, we can print them to the screen:
```{r print-st}
g$summary.trees[[10]]
e$summary.trees[[10]]
a$summary.trees[[10]]
...
```{r check-equality}
all.equal(g$summary.trees[[20]], e$summary.trees[[20]])
all.equal(g$summary.trees[[20]], a$summary.trees[[20]])
```

In this case, all three 20-node summary trees are the same. In other words, for $k=20$, the greedy algorithm and the approximate algorithm returned the maximum entropy summary tree (which is always the result of the exact algorithm).
...
Also note that the `greedy()` and `optimal()` functions return a list with three objects: `summary.trees`, which we have seen above and contains the list of summary trees for $k = 1, 2, ..., K$, as well as two additional objects: `data` and `tree`. The `data` object is a data.frame containing the input data (node, parent, weight, and label) after having been sorted by level, and then by parent. The `tree` object is a more compact representation of the tree's structure: a matrix with three columns, the first identifying the node ID of each parent in the tree, and the second and third columns containing the node IDs of the first and last child of each parent (which is possible because the nodes have been sorted within each level by parent).
