...
- Is there an off-the-shelf solution for (1) verifying that a given input structure is, indeed, a rooted, node-weighted tree, and (2) computing a stable version of that tree, or a so-called 'canonical' version of it?
...
The `d3tree()` function takes five arguments:

1. The first one, `tree.list`, is the list of $K$ summary trees that were computed by either `greedy()` or `optimal()`.

2. The second argument, `out.dir`, is the directory from which to serve the browser-based visualization.

3. `open.browser`

4. `labels` is a character vector of length $n$ the contains the labels of all the nodes in the original tree.

5. `tree` is the matrix that is output by `order.nodes()` that contains the list of all unique parents in the tree, and the sequence of indices of each of their respective child nodes.

We call the function `d3tree()` below and our default browser opens and shows the interactive visualization:

```{r d3tree}
d3tree(tree.list = e, 
       out.dir = tempfile(), 
       open.browser = interactive(), 
       labels = tmp$data[, "label"], 
       tree = tmp$tree)
```
